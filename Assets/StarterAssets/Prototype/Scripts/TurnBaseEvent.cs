    ...
    public class TurnBaseManager : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (_turnBaseMode)
            {
                ...
                float walkCost = (distance / 100) * combatTurnBase._maxAction;

                combatTurnBase._action = combatTurnBase._action - (int)walkCost;
            }
        }

        void TurnHandler()
        {
            queue = queue % charQueue.Count;
            for (int i = 0; i < charQueue.Count; i++)
            {
                charQueue[i].enabled = i == queue ? true : false;
